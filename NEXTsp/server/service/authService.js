...
const loginUserService = async ({ accountName, email, password }) => {
    ...
    const accessToken = jwt.sign({ userId: user._id,userName:user.accountName }, process.env.ACCESS_TOKEN_SECRET,{ expiresIn: '1h' });
...
const loginAdminService = async ({ accountName, password }) => {
    ...
    const accessToken = jwt.sign({ adminId: admin._id,adminName:admin.accountName }, process.env.ACCESS_TOKEN_SECRET,{ expiresIn: '1h' });
